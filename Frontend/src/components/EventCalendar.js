...
const EventCalendar = () => {
  ...
  const handleApproveEvent = (eventId) => {
    axios
      .put(`http://localhost:5000/api/events/${eventId}/approve`) // Replace with your API endpoint for approval
      .then((response) => {
        // Update the events state to reflect the approval status
        ...
        {events
          ...
          .map((event) => (
            <div key={event._id} onClick={() => handleEventClick(event)}>
              ...
              {!event.approved && (
                <button onClick={() => handleApproveEvent(event._id)}>Approve Event</button>
              )}
              {event.approved && <p>Status: Approved</p>}
            </div>
          ))}
      </div>
    </div>
